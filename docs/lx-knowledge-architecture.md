# How Henry uses the LX knowledge — architecture & LX-RAG lineage

Date: 2026-08-07. This is the canonical description of Henry's knowledge system and its
explicit mapping to the LX-RAG that GrowthX built in gx-backend (`learning_chunks` +
...
```
gx-prod Mongo (read-only, DB_STRING from gx-backend .env at runtime)
   │  export: learningchunks (LLM-chunked by LX-RAG) + video transcripts + text modules
   ▼
knowledge/raw/            ← markdown + jsonl, gitignored (GrowthX IP, never leaves machine)
...
INJECTION — only when the turn's domain matches (zero-LLM router), as a labeled block:
   "--- GrowthX knowledge (tried & tested founder playbooks) ---"
   cards first, raw chunks as depth; ≤2 chunks/module; ~6k char budget; source attribution
```

## 2. Element-by-element mapping to the backend LX-RAG

| LX-RAG (gx-backend) | Henry | Why the swap |
|---|---|---|
...
## 3. What Henry adds beyond LX-RAG

1. **Strategy cards** — a distilled precision layer LX-RAG doesn't have: atomic playbook
...
3. **Usage reinforcement** — `markUsed`/`reinforce` on every recall: cards that keep proving
   useful rank higher; feeds salience like LX-RAG's planned usage signals.
...
5. **Two-store separation** — personal memory (decays/supersedes) vs knowledge (versioned,
   never decays); LX-RAG only has the corpus store.
...
- Corpus and DB are **local-only forever** (GrowthX IP): `knowledge/`, `data/knowledge.db`
