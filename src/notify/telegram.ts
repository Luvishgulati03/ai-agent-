...
/**
 * SCOPE-GUARD: this is Dad's operator-notification channel ONLY. `chat_id` is always read
...
export async function sendTelegram(config: HenryConfig, text: string): Promise<boolean> {
  ...
  try {
    const body = text.length > TELEGRAM_MAX_CHARS ? `${text.slice(0, TELEGRAM_MAX_CHARS - 1)}…` : text;
